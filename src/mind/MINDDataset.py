...
class MINDTrainDataset(Dataset):
    def __init__(
        ...
        self.behavior_df = self.behavior_df.with_columns(
            [
                pl.col("impressions")
                .map_elements(lambda v: [i for i, imp_item in enumerate(v) if imp_item["clicked"] == 1])
                .alias("clicked_idxes"),
                pl.col("impressions")
                .map_elements(lambda v: [i for i, imp_item in enumerate(v) if imp_item["clicked"] == 0])
